...
public class KafkaTracesEventEmitter implements TracesEventEmitter, AutoCloseable {
  private final Consumer<byte[], byte[]> consumer;
  private final Duration pollTimeout;
  private final Map<TopicPartition, OffsetAndMetadata> pendingCommit = new HashMap<>();

  public KafkaTracesEventEmitter(
  ...
  @Override
  public List<TracesEvent> next(int ignoredBatchSize) {
    if (!pendingCommit.isEmpty()) {
      throw new IllegalStateException("The previous traces event batch must be committed first");
    }
    ...
    consumer
        ...
        .forEach(
            record -> {
              batch.add(new TracesEvent(record.value()));
              pendingCommit.merge(
