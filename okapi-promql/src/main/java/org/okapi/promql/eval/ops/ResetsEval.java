...
public final class ResetsEval implements Evaluable {
  private final FunctionExpr fn;

  public ResetsEval(FunctionExpr fn) {
    this.fn = fn;
  }
  ...
  public ExpressionResult eval(EvalContext ctx) throws EvaluationException {
    if (fn.args.size() != 1) {
      throw new IllegalArgumentException("resets(range-vector) expects one arg");
    }
    var argRes = fn.args.get(0).lower().eval(ctx);
    if (!(argRes instanceof RangeVectorResult rv)) {
      throw new IllegalArgumentException("resets expects range vector");
    }
    ...
    for (SeriesWindow w : rv.data()) {
      ...
      for (long t = ctx.startMs; t <= ctx.endMs; t += ctx.stepMs) {
        ...
        float value = resetsInWindow(s, winStart, evalTimeMs);
  ...
  private float resetsInWindow(Scan scan, long start, long end) {
    if (scan instanceof GaugeScan gs) {
      return resetsInWindow(gs.getTimestamps(), toFloatList(gs.getValues()), start, end);
    }
    if (scan instanceof SumScan ss) {
      return resetsInWindow(ss.getTs(), toFloatList(ss.getCounts()), start, end);
    }
    return Float.NaN;
  }

  private float resetsInWindow(List<Long> ts, List<Float> vals, long start, long end) {
  ...
  private long inferRangeFromArg(LogicalExpr arg) {
    ...
    throw new IllegalStateException("resets requires range selector or subquery");
  }
